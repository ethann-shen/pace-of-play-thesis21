...
The baseline hierarchical logistic model (without any pace metrics) is as follows:
...
bind_cols(glmer.fit.baseline.coef, glmer.fit.pace.coef) %>% 
  ...
  kable_styling(latex_options = c("hold_position"), font_size = 11.5) %>%
...
The baseline multinomial logistic model (without any pace metrics) is as follows:
...
The baseline multinomial model reports an accuracy of `r multinom_output[1,]$ACC_test` on the test data while the best performing pace model, the one with $\Delta_{ij}^{FZ}$, reports a slightly higher accuracy of `r multinom_output[4,]$ACC_test`. The best modified model with a pace metric actually performs slightly worse on the train set than the baseline model. Once again, we see that the addition of pace metrics do not significantly improve the predictive power of the model on the test set. 
...
kbl(caption = "Coefficients obtained from models 2.1 and 2.2.", escape=F, booktabs = TRUE,
  ...
  kable_styling(latex_options = c("hold_position"), font_size = 13) %>%

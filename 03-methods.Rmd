...
After creating the pace-of-play metrics, we wanted to evaluate their effectiveness when used as variables in models that predict the outcome of a game. We implemented models with and without the pace metrics to determine if the models with the pace metrics achieve a higher accuracy. When selecting variables, we did not include any traditional performance-based features, such as number of shots or corners taken. **I can't think of a good reason why we don't include peformance based stats** We only considered pre-game variables, ones with information that can be gathered before a game is played. 
...
The final logistic hierarchical models are shown in 1.1 and 1.2. $Y_{ij}$ is the outcome (win or loss) of a game in league $k$ (is $k$ necessary to add to model/variables?) when home team $i$ plays away team $j$ and $P_{ij}$ is the probability that home team $i$ defeats away team $j$. $\alpha_k$ represents the random intercept term for *HomeTeam* $k$. The only difference between models 1.1 and 1.2 is the addition of the variable $\Delta_{ij}OffZones$ in 1.2. 
